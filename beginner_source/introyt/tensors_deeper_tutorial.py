...
# ``empty()`` reserves storage for a tensor with 3 rows and 4 columns.
# It does not write any specific values into that storage.
x = torch.empty(3, 4)
print(type(x))  # Confirms that PyTorch returns a ``torch.Tensor`` object.
print(x)  # The contents are whatever happened to be in memory already.
...
# ``zeros()`` initializes every element, which is safer than ``empty()``
# when you need predictable values from the start.
zeros = torch.zeros(2, 3)
print(zeros)

# ``ones()`` has the same shape argument style, but fills the tensor with 1s.
ones = torch.ones(2, 3)
print(ones)

# Seeding makes the following random draw repeatable across runs.
torch.manual_seed(1729)
# ``rand()`` samples floating point values uniformly from the interval [0, 1).
...
torch.manual_seed(1729)
random1 = torch.rand(2, 3)  # First draw after this seed.
print(random1)

random2 = torch.rand(2, 3)  # Second draw from the same seeded sequence.
print(random2)

# Resetting to the same seed restarts the same random sequence.
torch.manual_seed(1729)
...
x = torch.empty(2, 2, 3)
print(x.shape)  # ``torch.Size([2, 2, 3])`` describes each dimension.
print(x)

# ``empty_like()`` copies the shape and dtype from ``x`` but leaves values
# uninitialized, just as ``empty()`` does.
empty_like_x = torch.empty_like(x)
print(empty_like_x.shape)
print(empty_like_x)

# ``zeros_like()``, ``ones_like()``, and ``rand_like()`` are convenient when
# a later operation needs a tensor that conforms to an existing tensor.
...
#
# The last way to create a tensor that we will cover is to specify its data
# directly from a PyTorch collection:
#

# Nested Python lists become a 2-D tensor; each inner list is one row.
some_constants = torch.tensor([[3.1415926, 2.71828], [1.61803, 0.0072897]])
print(some_constants)

# A flat tuple creates a 1-D tensor.
some_integers = torch.tensor((2, 3, 5, 7, 11, 13, 17, 19))
print(some_integers)

# PyTorch accepts a mix of tuples and lists as long as the nested structure
...
# The ``dtype`` argument controls how values are stored in memory.
a = torch.ones((2, 3), dtype=torch.int16)
print(a)

# This tensor starts as float64 and is then scaled so truncation is visible
# in the conversion below.
b = torch.rand((2, 3), dtype=torch.float64) * 20.0
print(b)

# ``to()`` returns a tensor converted to the requested dtype. Floating point
# values are truncated when converted to an integer type.
c = b.to(torch.int32)
print(c)
...
# Scalar arithmetic is applied element by element across the tensor.
ones = torch.zeros(2, 2) + 1
twos = torch.ones(2, 2) * 2

# Operations can be chained; standard Python precedence rules still apply.
...
# Tensor-tensor arithmetic pairs elements by position when shapes match.
powers2 = twos ** torch.tensor([[1, 2], [3, 4]])
print(powers2)

# ``ones`` and ``fours`` are both 2 x 2, so addition happens element-wise.
fives = ones + fours
print(fives)

# Multiplication also happens element-wise rather than as matrix multiplication.
dozens = threes * fours
print(dozens)
...
# The right-hand tensor has one row and four columns. Broadcasting reuses that
# single row for both rows of ``rand``.
...
# ``torch.rand(3, 2)`` behaves like shape ``(1, 3, 2)`` here, so its values
# are reused across the first dimension of ``a``.
b = a * torch.rand(3, 2)  # 3rd & 2nd dims identical to a, dim 1 absent
print(b)

# The final dimension has size 1, so each generated value is reused across
# the two columns in the matching row.
c = a * torch.rand(3, 1)  # 3rd dim = 1, 2nd dim identical to a
print(c)

# Here the middle dimension has size 1, so each row receives the same pair of
# values across that dimension.
...
# common functions
# Start with values in [-1, 1) so absolute value, ceil, floor, and clamp
# produce visibly different results.
a = torch.rand(2, 4) * 2 - 1
print("Common functions:")
print(torch.abs(a))  # Distance from zero for each element.
print(torch.ceil(a))  # Smallest integer greater than or equal to each value.
print(torch.floor(a))  # Largest integer less than or equal to each value.
print(torch.clamp(a, -0.5, 0.5))  # Limit every value to the given range.

# trigonometric functions and their inverses
angles = torch.tensor([0, math.pi / 4, math.pi / 2, 3 * math.pi / 4])
sines = torch.sin(angles)  # Compute sine for every angle in radians.
inverses = torch.asin(sines)  # Recover the arcsine values element-wise.
print("\nSine and arcsine:")
print(angles)
print(sines)
print(inverses)
...
# XOR compares the binary representation of each pair of integer elements.
...
# reductions:
print("\nReduction ops:")
print(torch.max(d))  # returns a single-element tensor containing the maximum
...
print("\nb:")
print(b)
print(b.sin_())  # the underscore marks this as an in-place operation
...
print(b)
print("\nAfter adding:")
# ``add_()`` writes the result into ``a`` itself and also returns ``a``.
print(a.add_(b))
print(a)
print(b)
print("\nAfter multiplying")
# ``mul_()`` squares each element of ``b`` because ``b`` is multiplied by
# itself in place.
print(b.mul_(b))
print(b)
...
print(c)
# ``out=c`` tells PyTorch to store the matrix multiplication result in the
# already allocated tensor ``c``.
...
a = torch.ones(2, 2)
b = a  # ``b`` is another reference to the same tensor object as ``a``.
...
a = torch.ones(2, 2)
b = a.clone()  # ``clone()`` allocates a new tensor and copies the data.
...
# ``clone()`` preserves autograd tracking, so ``b`` stays connected to the
# computation history that produced ``a``.
b = a.clone()
print(b)

# ``detach()`` removes the autograd history first; ``clone()`` then makes an
# independent tensor that does not track gradients.
c = a.detach().clone()
print(c)
...
if torch.accelerator.is_available():
    # This branch runs only when PyTorch detects at least one supported
    # accelerator backend, such as CUDA, MPS, MTIA, or XPU.
...
# GPU” is shortened to, “moving the data to the GPU”.)
#
...
if torch.accelerator.is_available():
    # Creating the tensor directly on the accelerator avoids creating it on
    # CPU first and then copying it to accelerator memory.
...
# Keep the selected device in one variable so later tensor creation and
# movement use the same target consistently.
my_device = (
...
# Passing ``device=my_device`` creates the tensor in the right memory location
# immediately.
...
y = torch.rand(2, 2)  # created on CPU by default
y = y.to(my_device)  # returns a tensor on ``my_device``
...
# Add a leading batch dimension, changing the shape from C x H x W to
# N x C x H x W where N is 1.
b = a.unsqueeze(0)
...
# Remove the leading dimension of size 1 so the tensor becomes a plain
# 20-element vector.
b = a.squeeze(0)
print(b.shape)
print(b)
...
# This has no effect because dimension 0 has size 2, not size 1.
d = c.squeeze(0)
print(d.shape)
...
# Change ``b`` from shape ``(3,)`` to ``(3, 1)`` so the last two dimensions
# can broadcast against the ``(3, 2)`` tail of ``a``.
...
# The trailing underscore means this call modifies ``batch_me`` itself.
...
# Flatten all three dimensions into one long vector. The product of the new
# dimensions must equal the original element count: 6 * 20 * 20.
input1d = output3d.reshape(6 * 20 * 20)
print(input1d.shape)

# You can also call reshape as a function in the torch module.
...
# NumPy starts with a 2 x 3 array of 64-bit floating point ones.
numpy_array = np.ones((2, 3))
print(numpy_array)

# ``from_numpy()`` creates a tensor that views the same memory as the ndarray.
...
# ``numpy()`` exposes the tensor storage as a NumPy ndarray when the tensor is
# on CPU and has a dtype/layout NumPy can represent.
...
numpy_array[1, 1] = 23  # Change the ndarray...
print(pytorch_tensor)

pytorch_rand[1, 1] = 17  # Change the tensor...
print(numpy_rand)
