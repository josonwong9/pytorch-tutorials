...
This section applies when Codex is running inside the GitHub Actions workflow (`Codex.yml`).
...
| Timeout | 60 minutes |
| Model | `Codex-opus-4-6-v1` via AWS Bedrock |
...
Codex is invoked when a user mentions `@Codex` in a PR comment or PR review comment. The triggering comment is passed as the prompt. Respond directly to what the user asked — do not perform unrequested actions.
...
- `AGENTS.md` - Project structure and coding style
